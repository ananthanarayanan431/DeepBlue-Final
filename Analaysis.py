...
from CheatCode import fiscal_year,fiscal_year_attributes
from CheatCode import strat_outlook,strat_outlook_attributes
from CheatCode import risk_management,risk_management_attributes
from CheatCode import innovation, innovation_attributes
...
def Analysis():
    ...
    st.info("""
    Begin by uploading the annual report of your chosen company in PDF format. Afterward, click on 'Process Document'. 
    Once the document has been processed, tap on 'Analyze Report' and the system will start its magic. 
    After a brief wait, you'll be presented with a detailed analysis and insights derived from the report for your reading.
    """)
    ...
    def GetVectorStore(nodes):
        ...
        if not os.path.exists(loc):
            index = VectorStoreIndex(nodes=nodes, service_context=service_context)
            index.storage_context.persist(persist_dir=loc)
        else:
            store = StorageContext.from_defaults(persist_dir=loc)
            index = load_index_from_storage(store)
        return index

    def GenerateInsight(engine, insight_name, section_name, output_format):
        template = """
        You are tasked with generating {insight_name} insight about the company for the {section_name} section from the annual report of the company:

        Given below is the output format, which has the subsections.
        Must use bullet points.

        Always use indian rupee symbols (₹) for money values, and round it off to millions or billions accordingly
        ...
        formatted_input = prompt_template.format(insight_name=insight_name, section_name=section_name,
                                                 output_format=output_format)

        response = engine.query(formatted_input)
        return response.response

    def ReportInsight(engine, section_name, fields_to_include, section_num):
        fields = None
        attribs = None
        if section_num == 1:
            fields = fiscal_year
        ...
        for i, field in enumerate(attribs):
            if fields_to_include[i]:
                response = GenerateInsight(engine, field, section_name, str({field: fields[field]}))
                ins[field] = response
        return {
            "insights": ins
        }
    ...
    # st.session_state.performance_highlights = None
    # st.session_state.major_events = None
    # st.session_state.challenges_encountered = None
    # st.session_state.strategic_initiatives = None
    # st.session_state.market_outlook = None
    # st.session_state.product_roadmap = None
    # st.session_state.risk_factors = None
    # st.session_state.risk_mitigation = None
    # st.session_state.r_and_d_activities = None
    # st.session_state.innovation_focus = None
    ...
    if st.sidebar.button("Process Document"):
        with st.spinner("Processing Document..."):
            nodes = ProcessPDF(pdfs)
            st.session_state.index = GetVectorStore(nodes)
            st.session_state.process_doc = True

        st.success("Document Processsed!")

    if st.session_state.process_doc:
        col1, col2 = st.columns([0.25, 0.75])
        with col1:
            st.write("""### Select Insights""")
            with st.expander("**Fiscal Year Highlights**", expanded=True):
                performance_highlights = st.checkbox("Performance Highlights")
                major_events = st.checkbox("Major Events")
                challenges_encountered = st.checkbox("Challenges Encountered")
            ...
            with st.expander("**Strategy Outlook and Future Direction**", expanded=True):
                strategic_initiatives = st.checkbox("Strategic Initiatives")
                market_outlook = st.checkbox("Market Outlook")
                product_roadmap = st.checkbox("Product Roadmap")
            ...
            with st.expander("**Risk Management**", expanded=True):
                risk_factors = st.checkbox("Risk Factors")
                risk_mitigation = st.checkbox("Risk Mitigation")

                risk_management_list = [risk_factors, risk_mitigation]

            with st.expander("**Innovation and R&D**", expanded=True):
                r_and_d_activities = st.checkbox("R&D Activities")
                innovation_focus = st.checkbox("Innovation Focus")
        ...
        with col2:
            if st.button("Analyze Report"):
                ...
                with st.status("**Analyzing Report...**"):
                    if any(fiscal_year_highlights_list):
                        ...
                        response = ReportInsight(engine, "Fiscal Year Highlights", fiscal_year_highlights_list, 1)
                    ...
                    if any(strategy_outlook_future_direction_list):
                        ...
                        response = ReportInsight(engine, "Strategy Outlook and Future Direction",
                                                 strategy_outlook_future_direction_list, 2)
                    ...
                    if any(risk_management_list):
                        ...
                        response = ReportInsight(engine, "Risk Management", risk_management_list, 3)
                    ...
                    if any(innovation_and_rd_list):
                        ...
                        response = ReportInsight(engine, "Innovation and R&D", innovation_and_rd_list, 4)
            ...
            tab1, tab2, tab3, tab4 = st.tabs(
                ["Fiscal Year Highlights", "Strategy Outlook and Future Direction", "Risk Management",
                 "Innovation and R&D"])
